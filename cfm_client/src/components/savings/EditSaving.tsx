...
import { get_saving_by_id } from "../../redux/actions/saving/saving"
...
function EditSaving({ get_categories, categories, saving, walletDetail, get_saving_by_id,
}) {
    const params = useParams();
    const wallet_id = Number(params.id);
    const saving_id = Number(params.idSaving)
    ...
    useEffect(() => {
        if (!dataFetched) {
            get_categories()
            get_saving_by_id(saving_id)
            get_wallet_by_id(wallet_id)
            setDataFetched(true)
        }
        saving ? formData.amount = String(saving.amount) : <></>
        saving ? formData.date = saving.date : <></>
        saving ? formData.description = saving.description : <></>
        saving ? setCategorySelected(saving.category) : <></>
    }, [saving, walletDetail])

    walletDetail && saving ? MAX = Number(walletDetail.balance) + Number(saving.amount) : <></>
    ...
    const handleSubmit = (e) => {
        ...
        if (FormDataToSend.amount > MAX || walletDetail.balance < 0) {
            ...
            const fetchData = async () => {
                ...
                try {
                    const res = await axios.put(`http://127.0.0.1:8000/api/wallet/${wallet_id}/saving/${saving_id}/${categoryId}/update`, FormDataToSend, config)
                    if (res.status === 204) {
                        toast.success('Ahorro modificado!')
                        setTimeout(() => {
                            navigate(`/bolsillo/${wallet_id}/ahorros`)
                        }, 1500);
                    ...
                    <div className="border-b border-gray-900/10 pb-4">
                        <div className="mt-2 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                ...
                                <div className="mt-2">
                                    ...
                                    <div className="grid mt-1 rounded-md sm:max-w-md">
                                        ...
                                        <p className="mt-3 text-md leading-6 text-zinc-300">
                                            Establece la cantidad del ahorro.
                                        </p>
                                        <input
                                            type="number"
                                            id="amount"
                            ...
                            <div className="col-span-full">
                                ...
                                <p className="mt-3 text-md leading-6 text-zinc-300">
                                    Selecciona el dia en que registrarás tu ahorro.
                                </p>
                                <input
                                    id="date"
                                    type="date"
                            ...
                            <div className="col-span-full">
                                ...
                                <p className="mt-3 text-md leading-6 text-zinc-300">
                                    Escribe una breve descripción sobre tu ahorro.
                                </p>
                                <textarea
                                    id="description"
                                    name="description"
