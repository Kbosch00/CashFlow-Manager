...
function NewSpending({ get_categories, categories, walletDetail, get_wallet_by_id
}) {
    ...
    const handleSubmit = (e) => {
        ...
        if (FormDataToSend.amount > walletDetail.balance || walletDetail.balance < 0) {
            ...
            const fetchData = async () => {
                ...
                try {
                    const res = await axios.post(`http://127.0.0.1:8000/api/wallet/${wallet_id}/${categoryId}/spendings/create`, FormDataToSend, config)
                    if (res.status === 201) {
