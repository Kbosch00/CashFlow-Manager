...
import { get_spending_list } from "../../redux/actions/spending/spending"
import { MdOutlineAddCircleOutline } from "react-icons/md";
import axios from "axios";
import toast, { Toaster } from "react-hot-toast";
import { get_wallet_by_id } from "../../redux/actions/wallet/wallet"

function SpendingList({ get_spending_list, spending_list, walletDetail, get_wallet_by_id }) {
    const params = useParams()
    const id = Number(params.id);

    useEffect(() => {
        get_spending_list(id)
        get_wallet_by_id(id)
    }, [id])

    function handleDelete(id_spending) {
        const config = {
            headers: {
        ...
        const fetchData = async () => {
            try {
                const res = await axios.delete(`http://127.0.0.1:8000/api/wallet/${id}/spending/${id_spending}/delete`, config)
                if (res.status === 204) {
        ...
        <Layout>
            ...
            {spending_list && spending_list ?
                <>
                    ...
                    <Link
                        className="flex ml-4 cursor-pointer mt-2 mb-4  w-min"
                        to={`/bolsillo/${id}/gastos/nuevo`}
                        onMouseEnter={() => {
                    ...
                    <div className="relative overflow-x-auto max-h-[630px] overflow-y-auto shadow-md sssm:rounded-lg mx-4">
                        <table className="w-full text-left text-gray-500 dark:text-gray-400">
                            <thead className="text-x uppercase text-base bg-violet-950 bg-opacity-60 text-zinc-50">
                                <tr>
                                    ...
                                    <th scope="col" className="px-6 py-3">
                                        <div className="flex items-center">
                                            Categoría
                                            <button><svg className="w-3 h-3 ml-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z" />
                                            </svg></button>
                                        </div>
                                    </th>
                            ...
                            <tbody className="text-base">
                                ...
                                {spending_list.map((spending, index) => (
                                    <tr key={index} className="cursor-default  bg-violet-950 bg-opacity-10 border-b border-white">
                                        ...
                                        <td className="py-4 pl-5">
                                            <Link to={`/bolsillo/${id}/gastos/${spending.id}/editar`} className="bg-indigo-700 hover:bg-indigo-500 transition duration-300 ease-in-out p-2 rounded-md font-medium text-zinc-50 uppercase">Editar</Link>
                                        </td>
                                        <td className="py-4 pl-5">
                                            <button onClick={() => handleDelete(spending.id)} className="bg-red-700 hover:bg-red-500 transition duration-300 ease-in-out p-2 rounded-md font-medium text-zinc-50 uppercase">Eliminar</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                : <div>
                    <div className="h-96 w-96 mx-auto"><img
                        src="/src/assets/images/empty-spending.png"
                        className="mb-2 h-96 rounded-t-lg object-cover"
                    />
                    </div>
                    <div className="grid grid-cols-1 justify-center text-center">
                        <span className="whitespace-nowrap text-4xl text-zinc-200 font-semibold text-center">Aun no has registrado un gasto</span>
                        <div className="flex gap-x-3 justify-center"><Link className="whitespace-nowrap font-medium mt-6 p-3 rounded-lg hover:text-zinc-50 hover:hover:bg-violet-700 text-2xl text-zinc-400 bg-purple-menu w-min  transition duration-300 ease-in-out" to={`/bolsillo/${id}/gastos/nuevo`}>Nuevo gasto</Link>
...
const mapStateToProps = (state) => ({
    spending_list: state.spending.spending_list,
